  ...
  useEffect(() => {
    if (editingActivity) {
      setFormData({
        ...
        published: editingActivity.published || false,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Thêm và chỉnh sửa các hoạt động ngoại khóa, tình nguyện, sự kiện</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) { setEditingActivity(null); resetForm(); } }}>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Ngày</Label><Input value={formData.date} onChange={(e) => setFormData(p => ({ ...p, date: e.target.value }))} placeholder="01/2024 hoặc 2023-2024" /></div>
                <div><Label>Địa điểm</Label><Input value={formData.location} onChange={(e) => setFormData(p => ({ ...p, location: e.target.value }))} placeholder="TP.HCM, Hà Nội..." /></div>
              </div>
              <div><Label>Link</Label><Input value={formData.link} onChange={(e) => setFormData(p => ({ ...p, link: e.target.value }))} placeholder="https://..." /></div>
              ...
              <div><Label>Nội dung chi tiết</Label>
                <RichTextEditor content={formData.content} onChange={(html) => setFormData(p => ({ ...p, content: html }))} placeholder="Mô tả chi tiết về hoạt động..." />
              </div>
              ...
              <TableRow key={activity.id}>
                ...
                <TableCell>
                  <div>
                    ...
                    {activity.date && <span className="text-xs text-muted-foreground ml-2">{activity.date}</span>}
                  </div>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Switch checked={activity.published || false} onCheckedChange={() => togglePublished.mutate({ id: activity.id, published: !activity.published })} />
                </TableCell>
                <TableCell>
                  <Switch checked={activity.featured || false} onCheckedChange={() => toggleFeatured.mutate({ id: activity.id, featured: !activity.featured })} />
                </TableCell>
                <TableCell>
